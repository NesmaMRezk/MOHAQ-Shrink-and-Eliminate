...
def requantize(x,accum_max_abs,number_of_bits,input_scale,w_scale,min,max,flag,stat=False,keep_threshold=False):
   ...
   # clamp(x, min, max, inplace=True)
    ...
    requant_scale,out = post_quantized_forward(accum_max_abs,x,accum_scale,number_of_bits,stat,keep_threshold)
...
#    if stat:
  ...
  #      scale =0
  #  else:
  #      scale = symmetric_linear_quantization_scale_factor(number_of_bits, max_abs)
  #  x = linear_quantize_clamp(x, scale, min, max, inplace=False)
    return x
...
def duplicate_vector(vector,str_list,Bi=False):
  ...
  #  trial=vector[0][0]
   # trial1=vector[0][1]
    ...
    return new_vector
def post_quantized_forward(accum_max_abs,accumulator,current_accum_scale,num_bits_acts,stat=False,keep_threshold=False):
       # accum_max_abs = get_tensor_max_abs(accumulator)
        if not keep_threshold:# and num_bits_acts==8:
        #    print (get_tensor_max_abs(accumulator))
            accum_max_abs=accum_max_abs
    ...
    def forward(self, x):
        ...
        for i in range(self.N_dnn_lay):
            ...
            if self.dnn_use_batchnorm[i] == False and self.dnn_use_laynorm[i] == False and quantize_flag and self.expand_ratio>1:
                x = self.drop[i](self.act[i](requantize(self.wx[i](duplicate_vector(x,split_indices)),self.activations_max[i],self.number_of_bits,1,w_scale,self.min,self.max,quantize_flag,self.stat,True)))
            if self.dnn_use_batchnorm[i] == False and self.dnn_use_laynorm[i] == False and quantize_flag and self.expand_ratio==1:
                x = self.drop[i](self.act[i](
                    requantize(self.wx[i](x),self.activations_max[i], self.number_of_bits, 1, w_scale,
                               self.min, self.max, quantize_flag,self.stat,True)))
...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.N_lstm_lay):
            ...
            if quantize_flag:
                wfx_out=requantize(wfx_out,self.activation_x_max[4*i],self.number_of_bits,1,w_scale[0],self.min,self.max,quantize_flag,self.stat)
                wix_out = requantize(wix_out,self.activation_x_max[4*i+1],self.number_of_bits,1,w_scale[1],self.min,self.max,quantize_flag,self.stat)
                wox_out =requantize(wox_out,self.activation_x_max[4*i+2],self.number_of_bits,1,w_scale[2],self.min,self.max,quantize_flag,self.stat)
                wcx_out = requantize(wcx_out,self.activation_x_max[4*i+3],self.number_of_bits,1,w_scale[3],self.min,self.max,quantize_flag,self.stat)
...
#printing happen in this loop!!!                # LSTM equations
                ...
                if quantize_flag and k>0:
                    ufh_out=requantize(ufh_out,self.activation_h_max[5*i],self.number_of_bits,1,w_scale[4],self.min,self.max,quantize_flag,self.stat)
                ...
                if quantize_flag and k>0:
                    it_out = requantize(it_out,self.activation_h_max[5*i+1],self.number_of_bits,1,w_scale[5],self.min,self.max,quantize_flag,self.stat)
                    iii=wix_out[k] +it_out
                  #  print(iii.max())
                ...
                if quantize_flag and k>0:
                    ot_out = requantize(ot_out,self.activation_h_max[5*i+2], self.number_of_bits,1,w_scale[6],self.min,self.max,quantize_flag,self.stat)
                ...
                if quantize_flag and k>0:
                    ct_out = requantize(ct_out,self.activation_h_max[5*i+3],self.number_of_bits,1,w_scale[7],self.min,self.max,quantize_flag,self.stat)
                ct = it * self.act[i](wcx_out[k] + ct_out) * drop_mask + ft * ct
                if self.quantize:
                    ct = requantize(ct,0, 16, 1,1,self.min, self.max,quantize_flag)

                ht = ot * self.act[i](ct)
                if quantize_flag:
                    ht = requantize(ht,self.activation_h_max[5*i+4], self.number_of_bits,1,1, self.min, self.max,quantize_flag,self.stat)
