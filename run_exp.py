...
from run_exp_test import set_bits_conf, set_bits_conf_LSTM, set_bits_conf_GRU
from utils import (
    check_cfg,
    create_lists,
    create_configs,
...
#Nesma: define a function for evaluation to use in statistics collection
...
def run_exp(cfg_file):
    ...
    retrain=strtobool(config["exp"]["retrain"])
    ...
    for ep in range(N_ep):
        ...
        for tr_data in tr_data_lst:
            ...
            for ck in range(N_ck_tr):
                ...
                model_files = {}
                for arch in pt_files.keys():
                    model_files[arch] = info_file.replace(".info", "_" + arch + ".pth.tar")
            ...
            for pt_arch in pt_files.keys():
                if os.path.exists(model_files[pt_arch]) and not os.path.exists(
                        out_folder + "/exp_files/final_" + pt_arch + ".pth.tar"):
                    copyfile(model_files[pt_arch], out_folder + "/exp_files/final_" + pt_arch + ".pth.tar")
    ...
    for pt_arch in pt_files.keys():
        if os.path.exists(model_files[pt_arch]) and not os.path.exists(out_folder + "/exp_files/final_" + pt_arch + ".pth.tar"):
            copyfile(model_files[pt_arch], out_folder + "/exp_files/final_" + pt_arch + ".pth.tar")
    print('saving pth')
    torch.save(arch_dict, "./saved.pth")
    print('saved pth')
...
def retrain(kaldi_path,out_folder,old_folder,precisions,n_layers,n_epochs):
    layers_bits = []
    proj_bits = [16]
    ...
    for i in range(n_layers*2):
        delta.append(0)
    set_bits_conf(config_main, precisions, delta, n_layers)
    weight_bits_str = ""
    proj_bits_str = ""
    for i in range(n_layers):
    ...
    config_main.set("architecture1", "arch_pretrain_file", out_folder+"/exp_files/final_architecture1.pth.tar")
    config_main.set("architecture1", "weights_bits",weight_bits_str)
    config_main.set("architecture1", "projection_bits","16," +proj_bits_str)
    config_main.set("architecture1", "quantize","True")
    config_main.set("architecture2", "quantize", "True")
    config_main.set("architecture2", "number_of_bits", str(precisions[len(precisions)-2]))
    config_main.set("architecture2", "arch_pretrain_file", out_folder+"/exp_files/final_architecture2.pth.tar")
    config_main.set("architecture3", "arch_pretrain_file", out_folder+"/exp_files/final_architecture3.pth.tar")
    config_main.set("exp","out_folder",out_folder)
    config_main.set("exp", "n_epochs_tr", str(n_epochs))
    config_main.set("exp", "retrain", "True")
    with open(cfg_file, 'w') as h:
        config_main.write(h)

    run_exp(cfg_file)

    config_main.set("architecture1", "arch_pretrain_file" , "none")
    config_main.set("architecture1", "quantize", "False")
    config_main.set("architecture2", "arch_pretrain_file", "none")
    config_main.set("architecture3", "arch_pretrain_file", "none")
    config_main.set("exp","out_folder",old_folder)
    config_main.set("exp", "n_epochs_tr", "24")
    config_main.set("exp", "retrain", "False")

    with open(cfg_file, 'w') as h:
        config_main.write(h)
def retrain_LiGRU(kaldi_path,out_folder,old_folder,precisions,n_layers,n_epochs):
    layers_bits = []
   # proj_bits = [16]
#    for i in range(n_layers):
#        layers_bits.append(precisions[i])
    ...
    # quantize the sru model
    quantize(out_folder, 1, 'liGRU', precisions, n_layers=n_layers,skip=1)
    # quantize the FC layer
    quantize(out_folder, 2, 'MLP', [precisions[len(precisions)-2]])
    quantize(out_folder, 3, 'MLP', [16])
   ...
   #        cfg_file = "cfg/TIMIT_baselines/TIMIT_SRU_fbank_6.cfg"
    ...
    #for i in range(n_layers*2):
    #    delta.append(0)
    set_bits_conf_GRU(config_main, precisions, n_layers)
    weight_bits_str = ""
    proj_bits_str = ""
    for i in range(n_layers*2):
        weight_bits_str = weight_bits_str + str(precisions[i])
...
def retrain_lstm(kaldi_path,out_folder,old_folder,precisions,n_layers,n_epochs):

    # quantize the sru model
    quantize(out_folder, 1, 'LSTM', precisions, n_layers=n_layers)
    # quantize the FC layer
    ...
    cfg_file = "cfg/TIMIT_baselines/TIMIT_LSTM_fbank_6.cfg"
    if not (os.path.exists(cfg_file)):
        sys.stderr.write("ERROR: The config file %s does not exist!\n" % (cfg_file))
        sys.exit(0)
    else:
    ...
    set_bits_conf_LSTM(config_main, precisions, n_layers)

    config_main.set("architecture1", "arch_pretrain_file", out_folder+"/exp_files/final_architecture1.pth.tar")
    config_main.set("architecture1", "weights_bits",str(precisions[0]) + "," + str(precisions[1]) + "," + str(precisions[2]) + "," + str(precisions[3])+ "," + str(precisions[4])+ "," + str(precisions[5])+ "," + str(precisions[6])+ "," + str(precisions[7]))
    config_main.set("architecture1", "quantize","True")
    config_main.set("architecture2", "quantize", "True")
    config_main.set("architecture2", "arch_pretrain_file", out_folder+"/exp_files/final_architecture2.pth.tar")
    config_main.set("architecture3", "arch_pretrain_file", out_folder+"/exp_files/final_architecture3.pth.tar")
...
#config_main.set("architecture2", "arch_pretrain_file", "none")
#config_main.set("architecture3", "arch_pretrain_file", "none")
#config_main.set("exp","out_folder",'exp/TIMIT_SRU_fbank_dnn_2')
#config_main.set("exp", "n_epochs_tr", "24")
#config_main.set("exp", "retrain", "False")
