...
#Nesma: added simple binary quantization
import torch
...
def linear_quantize(input, scale_factor, binary=False, inplace=False):
    if binary:
        return binary_quantize(input)
    if inplace:
...
def linear_quantize_clamp(input, scale_factor, clamp_min, clamp_max, binary=False,inplace=False):
    input_o=input
    output = linear_quantize(input, scale_factor,binary, inplace)
