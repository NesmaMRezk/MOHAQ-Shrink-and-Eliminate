...
#Nesma: updated file to work with Mxv not only convolutions
...
#-------------------------------------------------------------------------
def ocs_wts(weight_np, expand_ratio, axis=1, split_threshold=0.5, w_scale=None, grid_aware=False,Convolution=True):
 ...
 #   if Convolution:
    ...
    for c in range(ocs_channels):
        ...
        if split_idx < num_channels:
            ...
            #Nesma: to store the list in a file
            in_channels_to_copy_str=in_channels_to_copy_str+str(split_idx)+","

            orig_idx_dict[num_channels+c] = split_idx
        else:
            idx_to_copy = orig_idx_dict[split_idx]
            in_channels_to_copy.append(idx_to_copy)
            in_channels_to_copy_str = in_channels_to_copy_str + str(idx_to_copy)+","
            orig_idx_dict[num_channels+c] = idx_to_copy

    return weight_np, in_channels_to_copy,in_channels_to_copy_str
