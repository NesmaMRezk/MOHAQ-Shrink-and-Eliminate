...
out_folder='exp/TIMIT_SRU_fbank_dnn_l3'
...
class MyProblem(Problem):
    ...
    def __init__(self):
        super().__init__(n_var=12, n_obj=2, n_constr=2, xl=1, xu=4, type_var=np.int,elementwise_evaluation=True)
        self.count = 0
    def _evaluate(self, x  , out, *args, **kwargs):
        ...
        if not play:
            ...
            wer,s = run_inference_for_optimization(kaldi_path,out_folder,x,n_layers=3, test=False)

            f2 = -(75900 * get_speedup_bitfusion(x[0], x[3])  + 844800 * get_speedup_bitfusion(x[1], x[4]) +
                   844800 * get_speedup_bitfusion(x[2], x[5]) +
                   281600 * get_speedup_bitfusion(x[6], x[8]) + 281600 * get_speedup_bitfusion(x[7], x[9]) +
                    2094400 * get_speedup_bitfusion(x[10],x[11])) / 5637500 / 2
            mem_size = compute_mem_3(x)
            wer_q=0
            beacon_index=0
            min_distance=0
            wer_t = wer
            if self.count>80:
                wer_test, s = run_inference_for_optimization(kaldi_path, out_folder, x, n_layers=3, test=True)
                wer_1=run_beacon_v3(x,1,test=False)
                wer_1_test = run_beacon_v3(x, 1, test=True)

                wer_2 = run_beacon_v3(x, 2,test=False)
                wer_2_test = run_beacon_v3(x, 2, test=True)

                wer_3= run_beacon_v3(x, 3,test=False)
                wer_3_test=run_beacon_v3(x, 3,test=True)
                ...
                wer_3=0
                wer_3_test=0
            distance_0=compute_distance_3(x,beacons_3[0])
            distance_1=compute_distance_3(x,beacons_3[1])
            distance_2 = compute_distance_3(x, beacons_3[2])
            distance_3 = compute_distance_3(x, beacons_3[3])
...
#   if i>-1:
n_gen=40
for i in range (10,n_gen-1,5):
    excel_log = xlsxwriter.Workbook('log_bitfusion_3_'+str(i)+'.xlsx')
    sheet_log = excel_log.add_worksheet()


    if i>0:

        method, = np.load("checkpoint_mem_bitfusion_beacons_create_3.npy", allow_pickle=True).flatten()
    ...
    np.save("checkpoint_mem_bitfusion_beacons_create_3", method)
    excel_log.close()
    plot = Scatter()
