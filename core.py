...
# October 2018
##########################################################
#Nesma: Updated file to save model parameters in a format that can be processed by NW distiller library
...
from distiller.data_loggers import QuantCalibrationStatsCollector, collect_quant_stats
#from distiller.data_loggers.collector import QuantCalibrationStatsCollector, collect_quant_stats
#import QuantCalibrationStatsCollector, collect_quant_stats
from distiller.quantization import get_tensor_max_abs
from tensorflow_core.python import deep_copy

from quantize import quantize, quantize_inline
...
def run_nn_refac01(
    ...
    def _read_chunk_specific_config(cfg_file):
        if not (os.path.exists(cfg_file)):
            sys.stderr.write("ERRORrr: The config file %s does not exist!\n" % (cfg_file))
            sys.exit(0)
        else:
    ...
    def _save_model(to_do, nns, multi_gpu, optimizers, info_file, arch_dict):
        if to_do == "train":
            for net in nns.keys():
                ...
                out_file = info_file.replace(".info", "_" + arch_dict[net][0] + ".pth.tar")
...
#Nesma function to copy gradients for quantization-aware training
def copy_grad(nn1,nn2):

    for net in nn1.keys():
        for net1, net2 in zip(nn1[net].named_parameters(), nn2[net].named_parameters()):
            net2[1].grad = net1[1].grad
    #        print(net2[1].grad )


def run_nn(
    data_name, data_set, data_end_index, fea_dict, lab_dict, arch_dict, cfg_file, processed_first, next_config_file,retrain=False,to_collect_statistics='False'
):
    ...
    if not (os.path.exists(cfg_file)):
        sys.stderr.write("ERRORxx: The config file %s does not exist!\n" % (cfg_file))
        sys.exit(0)
    else:
        config = configparser.ConfigParser()
        config.read(cfg_file)
    ...
    use_cuda = strtobool(config["exp"]["use_cuda"])
    multi_gpu = strtobool(config["exp"]["multi_gpu"])
    weights_bits=list(map(int, config["architecture1"]["weights_bits"].split(",")))
    fc_bits=int(config["architecture2"]["number_of_bits"])
    projection_bits=list(map(int, config["architecture1"]["projection_bits"].split(",")))
    ...
    #print(data_set[40:48, :].contiguous())
    ...
    [nns, costs] = model_init(inp_out_dict, model, config, arch_dict, use_cuda, multi_gpu, to_do)
    [nns_re, costs]=model_init(inp_out_dict, model, config, arch_dict, use_cuda, multi_gpu, to_do)
    [nns_copy_1, costs] = model_init(inp_out_dict, model, config, arch_dict, use_cuda, multi_gpu, to_do)
    [nns_copy_2, costs] = model_init(inp_out_dict, model, config, arch_dict, use_cuda, multi_gpu, to_do)
    #nns_re=nns
    ...
    for net in nns.keys():
        pt_file_arch = config[arch_dict[net][0]]["arch_pretrain_file"]
        #Nesma: might change when have more pth files
        if to_collect_statistics=='True':
            if not os.path.isdir(output_folder+net):
                os.makedirs(output_folder+net)
            collector.append(QuantCalibrationStatsCollector(nns[net]))
        ...
        if pt_file_arch != "none":
            ...
            if retrain:
                print('loading again')
                if ii==3:
                    checkpoint_load = torch.load(
                        'exp/TIMIT_liGRU_fbank/exp_files/final_architecture' + str(ii) + '.pth.tar')
              #      checkpoint_load = torch.load(
              #          'exp/TIMIT_LSTM_fbank_c4/exp_files/final_architecture' + str(ii) + '.pth.tar')

                else:
                    print('loading q '+str(ii))
                    #TODO check no problem
                    checkpoint_load=torch.load(output_folder+'/exp_files/quantize_TIMIT_architecture'+str(ii)+'.pth.tar')
                    if ii==1:
                        checkpoint_orig = torch.load(
                            output_folder + '/exp_files/quantize_TIMIT_architecture' + str(ii) + '.pth.tar')

                nns_re[net].load_state_dict(checkpoint_load["model_par"])
            else:
                nns_re[net]=nns[net]
        ii+=1
        if multi_gpu:
    ...
    seq_model = is_sequential_dict(config, arch_dict)
   # print(nns_re['SRU_layers'].sru.rnn_lst[1].weight[0])
    ...
    for i in range(N_batches):
        ...
        if to_do == "train":
            ...
            outs_dict["loss_final"].backward()
            if retrain:
                copy_grad(nns_re,nns)
        #    print('grad_copied')

           # for param in nns_re['SRU_layers'].sru.rnn_lst[1].weight[1]:
           # print(nns['SRU_layers'].sru.rnn_lst[1].weight[1].grad.data.sum())
            ...
            for opt in optimizers.keys():
                if not (strtobool(config[arch_dict[opt][0]]["arch_freeze"])):
            #        print(optimizers['SRU_layers'].param_groups[0]['params'][5])

                    optimizers[opt].step()
                  #  if opt == 'SRU_layers' and nns['SRU_layers'].sru.rnn_lst[1].weight.grad != None:
                    #    print(get_tensor_max_abs(nns_re['SRU_layers'].sru.rnn_lst[1].weight.grad))
                    #    print(get_tensor_max_abs(nns_re['SRU_layers'].sru.rnn_lst[1].weight))
                 #       print(get_tensor_max_abs(nns['SRU_layers'].sru.rnn_lst[1].weight.grad))
                 #       print(get_tensor_max_abs(nns['SRU_layers'].sru.rnn_lst[1].weight))
                   # if opt == 'MLP_layers' and nns_re['MLP_layers'].wx[0].weight.grad!=None:
                    #    print(get_tensor_max_abs(nns_re['MLP_layers'].wx[0].weight))
                    #print(optimizers['SRU_layers'].param_groups[0]['params'][5])
        ...
        if retrain:
            ...
            for net in nns_re.keys():

                    checkpoint = {}
                    checkpoint["model_par"] = nns[net].state_dict()

                    checkpoint["optimizer_par"] = optimizers[net].state_dict()
                    nns_copy_1[net].load_state_dict(checkpoint["model_par"])
                    #if ii==1:
                      #  nns_copy_2[net].load_state_dict(checkpoint_orig["model_par"])
                    #else:
                    nns_copy_2[net].load_state_dict(checkpoint["model_par"])
                   # torch.save(checkpoint, output_folder + '/exp_files/final_' + arch_dict[net][0] + ".pth.tar")
                   # torch.save(nns[net], output_folder + '/exp_files/checkpoint_' + arch_dict[net][0] + ".pth.tar")
                    #checkpoint_2 = copy(checkpoint)
                    # Nesma: might change when have more pth files
                    if ii == 1:

                   #     print(nns_re[net].sru.rnn_lst[1].weight[0])
                        #quantize_inline(nns_copy_1[net], nns_copy_2[net], 'LSTM', weights_bits, p_bits=[],checkpoint=checkpoint,n_layers=6)
                        #quantize_inline(nns_copy_1[net], nns_copy_2[net], 'SRU',weights_bits, projection_bits,checkpoint,n_layers=4)
                        quantize_inline(nns_copy_1[net], nns_copy_2[net], 'liGRU', weights_bits,
                                        checkpoint=checkpoint, n_layers=5,skip=1)
                #        print(nns_re[net].sru.rnn_lst[1].weight[0])
                    #    quantize(output_folder, ii, 'SRU', [8,8,8,8], [16,8,8,8])
                    elif ii == 2:

                        quantize_inline(nns_copy_1[net], nns_copy_2[net], 'MLP',[fc_bits] ,[],checkpoint)
                     #   quantize(output_folder, ii, 'MLP', [16])
            #        elif ii == 3:
             #           quantize(output_folder, ii, 'MLP', [16])
                        #quantize_inline(nns_copy_1[net], nns_copy_2[net], 'MLP', [16], [], checkpoint)
                    if ii!=3:
                        #checkpoint_load = torch.load(
                        #            output_folder+'/exp_files/final_architecture' + str(ii) + '.pth.tar')
                        nns_re[net].load_state_dict(checkpoint["model_par"])

                    #checkpoint_load = torch.load(
                    #        output_folder + '/exp_files/quantize_TIMIT_architecture' + str(ii) + '.pth.tar')
             #       if ii==1:
                        #print(nns_re[net].sru.rnn_lst[0].weight[0])
              #          print(nns_re[net].wfx0.weight[0])
                    #nns_re[net].load_state_dict(checkpoint_load["model_par"])
                  #  if ii==1:
                  #      print(nns_re[net].sru.rnn_lst[0].weight[0])
                    ii += 1
        ...
        if to_do == "train":
           ...
           # print(loss_sum.cpu().item())
           # print(err_sum.cpu().item())
            if i == N_batches - 1:
                status_string = "Training | (Batch " + str(i + 1) + "/" + str(N_batches) + ")"
    ...
    # save the model
    if to_do == "train":
        # Nesma I modified here the line nns[net].statedict
        ii=1
        #nns_copy = copy(nns)
        for net in nns.keys():
            checkpoint = {}
            ...
            out_file = info_file.replace(".info", "_" + arch_dict[net][0] + ".pth.tar")
            torch.save(checkpoint, out_file)
            #Nesma: saving different formats to be used in NW distiller
            torch.save(checkpoint, output_folder+'/exp_files/final_' + arch_dict[net][0] + ".pth.tar")
         #   torch.save(nns[net], output_folder + '/exp_files/checkpoint_' + arch_dict[net][0] + ".pth.tar")

            if retrain:
                if ii == 1:
                    #quantize_inline(nns[net], nns_copy, 'SRU', [4,8,2,2], [16,16,16,2])
                   # print(nns_re[net].sru.rnn_lst[1].weight[0])
                   # quantize(output_folder, ii, 'SRU', weights_bits, projection_bits,n_layers=4)
                    quantize(output_folder, ii, 'liGRU', weights_bits, n_layers=5,skip=1)
                    #quantize(output_folder, ii, 'LSTM', weights_bits, n_layers=4)
                elif ii == 2:
                   # quantize_inline(nns[net], nns_copy, 'MLP',  [16])
                    quantize(output_folder, ii, 'MLP', [fc_bits])
                ii+=1

    if to_do == "forward":
        for out_name in forward_outs:
            post_file[out_name].close()
    print("error  "+str(err_tot))
    # Write info file
    with open(info_file, "w") as text_file:
        text_file.write("[results]\n")
        if to_do != "forward":
