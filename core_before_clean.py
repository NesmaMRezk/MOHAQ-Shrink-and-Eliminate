...
def run_nn(
    data_name, data_set, data_end_index, fea_dict, lab_dict, arch_dict, cfg_file, processed_first, next_config_file,to_collect_statistics='False'
):
    ...
    [nns, costs] = model_init(inp_out_dict, model, config, arch_dict, use_cuda, multi_gpu, to_do)
    #torch.save(nns,'/data/home/nesrez17/pytorch-kaldi/exp/TIMIT_LSTM_fbank/exp_files_test/nn.pth.tar')
    ...
    for net in nns.keys():
        ...
        #else: # not training
    #    pt_file_arch='exp/TIMIT_LSTM_fbank15//exp_files/final_architecture1.pth.tar'
        ...
        if pt_file_arch != "none":
            if use_cuda:
                ...
                checkpoint_load = torch.load(pt_file_arch, map_location="cpu")
            #print(checkpoint_load["model_par"])

            loaded=checkpoint_load["model_par"]
            nns[net].load_state_dict(checkpoint_load["model_par"])
            optimizers[net].load_state_dict(checkpoint_load["optimizer_par"])
            optimizers[net].param_groups[0]["lr"] = float(
                config[arch_dict[net][0]]["arch_lr"]
