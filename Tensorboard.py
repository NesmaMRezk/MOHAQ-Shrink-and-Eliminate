...
class Tensorboard:
    ...
    def log_histogram_int(self, tag, values, global_step, bins):
        counts, bin_edges = np.histogram(values, bins=bins)

        hist = tf.HistogramProto()
        hist.min = int(np.min(values))
        hist.max = int(np.max(values))
        hist.num = int(np.prod(values.shape))
        hist.sum = int(np.sum(values))
        hist.sum_squares = int(np.sum(values ** 2))
