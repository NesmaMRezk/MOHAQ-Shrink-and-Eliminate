...
first_gen_1=[17.2,16.3,15,17.8,17.2,42,17.3,34.5,19.3,18.4,16.3,22.4,34,15.2,16.8,17.1,16.2,22.9,16.4,17.9,16.7,15,38.7,37.9,17.1,19.2,15.8,30.7,17.3,15.8,19.1,20.1,17.2,18.6,16.3,40,40.2,18.1,18.5,38.3]
first_gen_2=[6.6,15,4.14,7.7,13.2,8.7,11.7,14.4,21.7,12.1,14.5,14.8,16.2,14.7,7.2,4.5,11.2,11,9.9,11.2,5.4,9.8,9.8,13.9,4.6,19.5,8.3,22.7,13.7,16.2,8,6.3,4.7,9.6,7,5.6,13,8.2,5.5,12.7]
...
class MyProblem(Problem):
    ...
    def __init__(self):
        ...
        super().__init__(n_var=n_precision+n_delta, n_obj=2, n_constr=1, xl=0, xu=3, elementwise_evaluation=True)
        self.count=0
    def _evaluate(self, x  , out, *args, **kwargs):
       ...
       # x2=[0,0,0,0,0,0,0,0,0]
        ...
        if self.count<40:
            f1=first_gen_1[self.count]
            f2 = first_gen_2[self.count]
            ...
            if wer<20:
                wer1,s=run_inference_for_optimization_LSTM('/usr/local/home/nesrez/kaldi', 'exp/TIMIT_LSTM_fbank_c4', x1, x2,
                                                    n_layers, False,1)
                wer2,s=run_inference_for_optimization_LSTM('/usr/local/home/nesrez/kaldi', 'exp/TIMIT_LSTM_fbank_c4', x1, x2,
                                                    n_layers, False,2)
                wer3,s=run_inference_for_optimization_LSTM('/usr/local/home/nesrez/kaldi', 'exp/TIMIT_LSTM_fbank_c4', x1, x2,
                                                    n_layers, False,3)
...
excel = xlsxwriter.Workbook('optimize_acc_mem_lstm_delta.xlsx')
excel_log = xlsxwriter.Workbook('log_acc__mem_lstm_delta.xlsx')

sheet_log=excel_log.add_worksheet()
stat = excel.add_worksheet()
mask=[]
for i in range(n_precision+n_delta):
    mask.append("int")
...
for i in range (n_gen-1):
    if i>-1:
        method, = np.load("checkpoint_delta_2.npy", allow_pickle=True).flatten()
    ...
    res = minimize(problem,
                   ...
                   verbose=True)
    np.save("checkpoint_delta_2", method)

    plot = Scatter()
    plot.add(res.F, color="red")
    plot.save("pareto_acc_mem_lstm_delta_2")
