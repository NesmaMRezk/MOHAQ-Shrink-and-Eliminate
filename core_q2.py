   ...
   # nns_re=nns
    ...
    for net in nns.keys():
        ...
        if pt_file_arch != "none":
            ...
            if retrain:
                checkpoint_load=torch.load('exp/TIMIT_SRU_fbank_y/exp_files/quantize_TIMIT_architecture'+str(ii)+'.pth.tar')
    ...
    for i in range(N_batches):
        ...
        if to_do == "train":
            ...
            if to_collect_statistics=='True':
                for nnet in nns.keys():

                    collect_quant_stats(nns[nnet],eval_for_stats, save_dir=output_folder+nnet)
            for opt in optimizers.keys():
                optimizers[opt].zero_grad()
           ...
           # print(nns['SRU_layers'].sru.rnn_lst[1].weight[1].grad.data.sum())
            ...
            #        print(optimizers['SRU_layers'].param_groups[0]['params'][5])
                    if opt=='SRU_layers' and nns['SRU_layers'].sru.rnn_lst[1].bias.grad!=None:
                        print(get_tensor_max_abs(nns['SRU_layers'].sru.rnn_lst[1].bias.grad))
                    optimizers[opt].step()
                    if opt == 'SRU_layers' and nns['SRU_layers'].sru.rnn_lst[1].bias.grad!=None:
                        print(get_tensor_max_abs(nns['SRU_layers'].sru.rnn_lst[1].bias.grad))
                    #print(optimizers['SRU_layers'].param_groups[0]['params'][5])
        ...
        if retrain:
            ...
            for net in nns_re.keys():
                ...
                # Nesma: might change when have more pth files
                if ii == 1:
                    quantize(output_folder, ii, 'SRU', [8, 4, 4, 4], [16, 4, 2, 4])
                elif ii == 2:
                    quantize(output_folder, ii, 'MLP', [4])
                elif ii == 3:
